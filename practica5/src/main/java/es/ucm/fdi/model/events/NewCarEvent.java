		...
		public Event parse(Map<String, String> map) {
			try {
				...
				if (faultProbability < 0){
					throw new IllegalArgumentException("Negative fault_probability on"+id);
				}else if (faultProbability > 1){
					throw new IllegalArgumentException("Imposible fault_probability on"+id);
				}
				...
				if (seed <= 0){
					throw new IllegalArgumentException("No positive seed on "+id);
				}
